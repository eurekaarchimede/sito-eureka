...
- **Feature wow (giugno 2026, terza ondata — brainstorm a giudici)**: hook globali `window.__eurekaWarp(n)` (inietta warpBoost, cap 40) e `window.__eurekaWish(dir)` (stella cadente direzionale dorata nello starfield, `wish:true` → render più marcato), oltre ai preesistenti `__eurekaBurst/__eurekaShockwave`.
  - ~~**Versa l'acqua / vasca di Archimede**~~ — RIMOSSA (giugno 2026): liquido giudicato "meccanico, poco realistico" + frizione del permesso gyro su iOS. Vedi sezione 4bis sotto per il rimpiazzo proposto.
  - **Accendi la lista** (`.cta.armed`/`.cta.lit`, `#cta-switch-wrap` > `#cta-switch`): nella CTA un interruttore hold-to-charge; tieni premuto (`pointerdown`, barra `--p` su rAF, `DUR 900ms`) → al pieno `light()`: `.lit` (h2 `bulbIgnite` + IG link riacceso) + `__eurekaWarp(30)` + shockwave + burst. Stato in sessionStorage (chi torna trova acceso). **Progressive**: senza JS o con RM la CTA è già accesa (`.armed` aggiunto solo da JS dim + mostra switch). Enter/Spazio = accende diretto (a11y).
  - **Plasma sull'interruttore** (`#cta-plasma`, canvas dentro `#cta-switch-wrap`, dietro il bottone): archi elettrici dorati frastagliati (midpoint-displacement ricorsivo, non onde sinusoidali) crepitano spontaneamente ogni 3-5s vicino all'interruttore "accendi la lista" — si vede SUBITO da fermo. Si intensificano (`Math.random() < dt*7`) durante `.charging`, un'unica scarica grande (`bigDischarge`, 5 archi) al momento dell'accensione, poi il canvas si ferma per sempre (one-shot). IO-gated su `.cta`, RM-gated, si auto-disabilita se la CTA è già `.lit` al load.
  ...
  - **Circuito a ponte** (`.bridge`, tra storia e illuminated — riempie lo slot lasciato dalla vasca): SVG con 2 contatti pulsanti; trascini il dito (pointer capture su nodeA) da un contatto all'altro → al rilascio, se entro 46 unità SVG dal secondo contatto (magnetismo generoso), `connect()`: filo si fissa dritto + lampadina `bulbIgnite` + shockwave + burst. Altrimenti il filo si ritira elastico (`retract()`, easing cubico via rAF, 220ms). Esito sempre binario e garantito — mai una simulazione fisica ambigua come la vasca. Ritoccabile all'infinito (`svg.classList.remove('connected')` al successivo pointerdown).
  - **Costellazioni che ricordi** (estende `#constellation` in `.candidati`): oltre alle ~28 stelle decorative, una stella-chiave per candidato (`keyStars`, posizione stabile per indice tra resize). Click su una card `.member` → `discoverStar(i)`: la stella si accende per sempre (oro, glow) e si collega alla stella scoperta più vicina (la costellazione personale cresce come un filo). Stato in `localStorage('eureka-constellation')`; chi torna trova i fili ricostruiti (catena greedy-nearest). A 14/14 scoperte: shockwave + burst + warp + messaggio "hai visto tutta la lista. eureka!".
  - **Stella cadente (flick)**: su touch/Android un colpetto secco del polso (`devicemotion.acceleration.x > 13`, edge-trigger + cooldown 650ms) lancia `__eurekaWish(dir)` nella direzione dello scatto + vibrazione + hint "esprimi un desiderio". Convive con lo shake-to-eureka (quello accumula energia, questo è il singolo spike). iOS skip (requestPermission) — niente più bridge al permesso vasca (rimossa), torna al design originale "iOS skip silenzioso".
